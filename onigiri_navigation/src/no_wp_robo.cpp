...
RoboCtrl::RoboCtrl() : it_(node), ac("/move_base", true)
{
    ...
    time_kimeuchi = ros::Time::now();

    // まず最初は決め打ちで適当に動く
    state_ = STATE_KIMEUCHI;
...
void RoboCtrl::moveRobo()
{
    ...
    if (state_ == STATE_KIMEUCHI)
    {
        static double time_before_next_run = 0.0;
        static int phase = 0;
        ...
        ROS_INFO("KIMEUCHI Phase: %d", phase);
    }
